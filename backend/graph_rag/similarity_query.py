...
from backend.graph_rag.config import EMBEDDINGS_MODEL, ENTITY_TYPES, RELATION_TYPES, client, neo4j_graph


SIMILARITY_SEARCH_PROMPT = f'''
...
def define_query(prompt: str, model: str = "gpt-4"):
    """Function to generate a query based on the user input using OpenAI's API."""
    completion = client.chat.completions.create(
        model=model,
        temperature=0,
        messages=[{"role": "system", "content": SIMILARITY_SEARCH_PROMPT}, {"role": "user", "content": prompt}],
    )
